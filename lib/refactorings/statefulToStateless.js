...
const wrapWithDecorators = (expr, decorators) => decorators.reduce((result, decorator) => t.callExpression(decorator.node.expression, [result]), expr)
...
export default function statefulToStateless(path) {
  ...
  let hasComponentDidUpdate = false
  let effectBody
  let effectTeardown
  ...
  path.traverse({
    ...
    ClassMethod(p) {
      ...
      } else if (p.node.key.name === "componentDidUpdate") {
        hasComponentDidUpdate = true
  ...
  if ((effectBody || effectTeardown)) {
    ...
    const importName = addNamed(path, "useEffect", "react")
    ...
    path.insertAfter(
      t.expressionStatement(
        t.assignmentExpression(
          "=",
          t.memberExpression(statelessComponent.declarations[0].id, p.node.key),
          p.node.value,
        ),
      ),
    )
