...
export const findImportDefaultSpecifier = (path) => {
  let match
...
export const findImportSpecifier = (path) => {
  let match
...
export function isUsingJSX(path) {
  let found = false
  path.traverse({
    JSXElement(p) {
      found = true
...
export function importReactIfNeeded(path) {
  if (isUsingJSX(path.find((p) => p.isProgram()))) {
